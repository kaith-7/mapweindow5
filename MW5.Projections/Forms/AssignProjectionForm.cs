...
namespace MW5.Projections.Forms
{
    ...
    public partial class AssignProjectionForm : MapWindowForm
    {
            ...
            : base(context)
        {
            ...
            if (context == null) throw new NullReferenceException("未传递引用");

            var database = context.Projections;
            if (database == null) throw new InvalidCastException("传递了无效的投影");
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            if (cs == null)
            {
                MessageService.Current.Info("未选择投影");
                return;
            }

            if (!LayersControl1.Filenames.Any())
            {
                MessageService.Current.Info("未选择文件");
                return;
            }
            ...
            {
                MessageService.Current.Info("选定的投影无法初始化");
                return;
            }
            ...
            if (count > 0)
            {
                MessageService.Current.Info(string.Format("此投影已指定给文件: {0}",
                    count));
            }

            if (report.MismatchedCount > 0)
            {
                report.ShowReport(projection, "以下文件未处理:", ReportType.Assignment);
            }
        ...
        private void ProjectionTreeView1_CoordinateSystemSelected(object sender, CoordinateSystemEventArgs e)
        {
            ...
            lblProjection.Text = cs != null ? "投影: " + cs.Name : "";
        }
