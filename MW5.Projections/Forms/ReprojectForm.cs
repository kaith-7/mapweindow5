...
namespace MW5.Projections.Forms
{
    public partial class ReprojectForm : MapWindowForm
    {
            ...
            : base(context)
        {
            ...
            if (database == null)
            {
                throw new InvalidCastException("传递了无效的投影实例。");
            }
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageService.Current.Warn("无法初始化所选的投影。");
                return;
            }
        ...
        private void DoReprojection(IEnumerable<string> filenames, ISpatialReference projection, bool inPlace)
        {
            ...
            report.ShowReport(projection, "重投影结果:", ReportType.Loading);
            ...
            if (count == 0)
            {
                MessageService.Current.Info("没有文件可添加到地图。");
                return;
            }
