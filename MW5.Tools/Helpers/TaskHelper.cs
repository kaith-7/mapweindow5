...
namespace MW5.Tools.Helpers
{
    ...
    internal static class TaskHelper
    {
        ...
        public static IEnumerable<string> GetDescription(this IGisTask task)
        {
            ...
            yield return "状态: " + task.Status.EnumToString();
            yield return "开始于: " + task.StartTime.ToLongTimeString();

            if (task.IsFinished)
            {
                yield return "完成于: " + task.FinishTime.ToLongTimeString();
                yield return "耗费时间: " + task.ExecutionTime;
            }
            ...
            if (gdalTool != null && gdalTool.OverrideOptions)
            {
                ...
                yield return "选项: " + gdalTool.EffectiveOptions;
