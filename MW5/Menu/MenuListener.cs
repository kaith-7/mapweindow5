...
namespace MW5.Menu
{
    public class MenuListener
    {
        ...
        public void RunCommand(string menuKey)
        {
            ...
            {
                if (menuKey == Plugins.Menu.MenuKeys.Quit)
                    return;
            ...
            switch (menuKey)
            {
                ...
                //    break;
                case MenuKeys.SelectionByAttribute:
                    FormHelper.ShowQueryBuilder(_context);
                    break;
                case MenuKeys.SelectionByLocation:
                    FormHelper.ShowSpatialQueryBuilder(_context);
                    break;
                case MenuKeys.GoToXY:
                    _context.Container.Run<GoToXYPresenter>();
                    break;
                case MenuKeys.SwitchFill:
                    _context.Map.ShapeLayerDrawFill = !_context.Map.ShapeLayerDrawFill;
                    break;

                //case "ShowPolygonPicture":
        ...
        private bool HandleCursorChanged(string itemKey)
        {
            ...
            switch (itemKey)
            {
                ...
                case MenuKeys.AttributesTool:
                    ...
                    return true;
                case MenuKeys.SwipeTool:
                    _context.Map.MapCursor = MapCursor.Swipe;
                    return true;
                //case "ShowPolygonPicture":
        ...
        private bool HandleHelpMenu(string itemKey)
        {
            switch (itemKey)
            {
                ...
                case MenuKeys.About:
                    ...
                    return true;
                case MenuKeys.Guide:
                    string filename = Directory.GetCurrentDirectory() + "//" + "SUNTOONGIS使用说明.pdf";
                    if (System.IO.File.Exists(filename))
                    {
                        System.Diagnostics.Process.Start(filename);
                    }else
                    {
                        FlexibleMessageBox.Show("找不到帮助文件");
                    }
                    return true;
            }
        ...
        private bool HandleLayerMenu(string itemKey)
        {
            switch (itemKey)
            {
                ...
                case MenuKeys.ClearLayers:
                    if (MessageService.Current.Ask("要删除地图中的所有图层吗?"))
                    {
        ...
        {
            MessageService.Current.Info("地图文档已保存: " + _projectService.Filename);
        }
        ...
        private void ZoomToBaseLayer()
        {
            if (_context.Map.Projection.IsEmpty)
            {
                MessageService.Current.Info("无法缩放到图层,未设置地图投影。");
            }
            else
            {
                ...
                {
                    MessageService.Current.Info("无法获取基础图层的范围。");
                }
                else
                {
                    if (!_context.Map.SetGeographicExtents(provider.GeographicBounds))
                    {
                        Logger.Current.Warn("缩放至基础图层：无法设置地理范围。");
                    }
                }
            }
        }
