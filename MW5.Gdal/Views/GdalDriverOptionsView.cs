...
namespace MW5.Gdal.Views
{
    ...
    public partial class GdalDriverOptionsView : GdalView, IGdalDriverOptionsView
    {
        ...
        public override void Initialize()
        {
            ...
            {
                _tabDriver = tabControlAdv1.AddTab("驱动", Resources.img_driver24);
            }
        ...
        private void GenerateDriverControls(Panel panel, DatasourceDriver driver)
        {
            ...
            if (options.Any())
            {
                ...
                _generator.GenerateIntoPanel(panel, driver.Name + "其他选项", parameters);

                parameters = _driverOptions.Where(o => options.Contains(o.Name));
                _generator.GenerateIntoPanel(panel, driver.Name + "主要选项", parameters);
            }
            else
            {
                // one section for all the others
                _generator.GenerateIntoPanel(panel, driver.Name + "选项", _driverOptions);
            }
