...
namespace MW5.Plugins.Symbology.Forms
{
    public partial class ChartStyleForm : MapWindowForm
    {
            ...
            : base(context)
        {
            ...
            cboValuesStyle.Items.Add("横向");
            cboValuesStyle.Items.Add("竖向");

            cboChartVerticalPosition.Items.Clear();
            cboChartVerticalPosition.Items.Add("当前图层上方");
            cboChartVerticalPosition.Items.Add("所有图层最上方");
        ...
        private bool ApplyOptions()
        {
            if (_charts.Fields.Count == 0)
            {
                if (_charts.Count == 0)
                {
                    MessageService.Current.Info("未选择任何字段, 将不会显示任何图表。");
                    return false;
                }

                if (MessageService.Current.Ask("未选择任何字段, 您要删除所有图表吗？"))
                {
                    _charts.Clear();
                    return true;
                }
        ...
        private void InitFields()
        {
            ...
            cboChartSizeField.Items.Add("<无>"); // default
            cboChartNormalizationField.Items.Add("<无>");
        ...
        private void SetChartsType()
        {
            if (_charts.Bars)
            {
                tabControl1.TabPages[1].Text = @"条形图";
            ...
            if (!_charts.Bars)
            {
                tabControl1.TabPages[1].Text = @"饼状图";
