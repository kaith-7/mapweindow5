...
namespace MW5.Plugins.Symbology.Helpers
{
    public static class FormHelper
    {
        ...
        public static void ShowQueryBuilder(IAppContext context, string expression = "", bool query = true)
        {
            var layer = context.Legend.Layers.Current;
            if (layer != null && (layer.LayerType == LayerType.VectorLayer || layer.LayerType == LayerType.Shapefile))
            {
                var model = new QueryBuilderModel(layer, expression, query);
                context.Container.Run<QueryBuilderPresenter, QueryBuilderModel>(model);
            }
        }

        public static void ShowSpatialQueryBuilder(IAppContext context)
        {
            var layers = context.Legend.Layers;
            if (layers != null && layers.Count(l => l.LayerType == LayerType.Shapefile || l.LayerType == LayerType.VectorLayer) > 0)
            {
                var model = new SpatialQueryBuilderModel(layers);
                context.Container.Run<SpatialQueryBuilderPresenter, SpatialQueryBuilderModel>(model);
            }
        }
