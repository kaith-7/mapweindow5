...
namespace MW5.Plugins.ShapeEditor.Context
{
    public partial class ContextMenuView : UserControl, IMenuProvider
    {
        ...
        private void VertexMenuOpening(object sender, CancelEventArgs e)
        {
            ...
            ctxSaveShape.Text = editing ? "保存图形修改" : "完成";
            
            bool hasChanges = editor.HasChanges 
                && ((editor.GeometryType == GeometryType.Polyline && editor.RawData.Length > 0) 
                 || (editor.GeometryType == GeometryType.Polygon && editor.RawData.Area > 0)); // 排除0长度和0面积
            //hasChanges = editor.EditorBehavior == EditorBehavior.VertexEditor ? editor.GeometryType == GeometryType.Polygon && editor.RawData.Area > 0 : hasChanges;
            if (hasChanges)
            {
                ctxSaveShape.Enabled = true;
                ctxClearEditor.Text = editing ? "暂存图形修改" : "取消";
            }
            else
            {
                ctxSaveShape.Enabled = false;
                ctxClearEditor.Text = "停止编辑";
            }
        }
        ...
        private void SelectionMenuOpening(object sender, CancelEventArgs e)
        {
            ...
            if (fs != null && fs.InteractiveEditing)
            {
                ...
                ctxEditLineInvert.Enabled = selectedCount > 0;
                ctxEditLineJoin.Enabled = selectedCount == 2;
                ctxEditLineExpand.Enabled = selectedCount == 1;
            }
