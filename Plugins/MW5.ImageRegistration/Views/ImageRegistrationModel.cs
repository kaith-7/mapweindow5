...
namespace MW5.Plugins.ImageRegistration.Views
{
    internal class ImageRegistrationModel
    {
        ...
        private void CheckImageFormat()
        {
            ...
            {
                MessageService.Current.Info("影像配准目前不支持BMP格式的影像。");
        ...
        {
            using (var dlg = new OpenFileDialog { Filter = GeoSource.RasterFilter, Title = "选择要配准的影像" })
            {
        ...
        private bool RemovePreviousWorldFile(string filename)
        {
            ...
            string msg = string.Format("文件已存在。{0}删除?", Environment.NewLine);
            ...
            if (result == DialogResult.Yes)
            {
                foreach (var name in names)
                {
                    ...
                    {
                        MessageService.Current.Info("删除文件失败: " + ex.Message);
                        return false;
                    }
                }
            }
