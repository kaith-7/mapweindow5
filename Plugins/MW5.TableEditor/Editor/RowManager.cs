...
namespace MW5.Plugins.TableEditor.Editor
{
    public class RowManager
    {
        ...
        public int RowIndex(int featIndex)
        {
            if (Filtered)
            {
                for (int i = 0; i < _filterMap.Length; i++)
                {
                    if (_filterMap[i] == featIndex)
                        return i;
                }
            }
            else
        ...
        public void SetSorting(int cmnIndex, bool ascending, IEnumerable<int> indices)
        {
            ...
            for (int i = 0; i < _sortingMap.Length && i < arr.Length; i++)
            {
                _sortingMap[i] = arr[i];
            }
