...
namespace MW5.Plugins.TableEditor.Views
{
    ...
    internal partial class TableEditorView : DockPanelControlBase, ITableEditorView
    {
        ...
        /// <summary>
        /// 汉化
        /// 通过资源汉化会导致panel大小异常,加载时滚动条和关闭按钮被遮挡，需要缩放才能显示
        /// </summary>
        void Localization()
        {
            lblNoLayers.Text = "属性表编辑器中没有打开的图层。";
            mnuAddField.Text = "添加字段";
            mnuAttributeExplorer.Text = "属性管理";
            mnuClearSelection.Text = "全不选";
            mnuClearSorting.Text = "清除排序";
            mnuDiscardChanges.Text = "放弃更改";
            mnuExportSelected.Text = "导出所选";
            mnuFind.Text = "查找";
            mnuFormatValues.Text = "值格式";
            mnuImportFieldDefinitions.Text = "导入字段定义";
            mnuInvertSelection.Text = "反选";
            mnuLayoutHorizontal.Text = "水平";
            mnuLayoutTabbed.Text = "分页式";
            mnuLayoutVertical.Text = "垂直";
            mnuQuery.Text = "生成查询";
            mnuRecalculateFields.Text = "重新计算";
            mnuReloadTable.Text = "重新加载表";
            mnuRemoveFields.Text = "删除字段";
            mnuReplace.Text = "替换";
            mnuSaveChanges.Text = "保存更改";
            mnuSelectAll.Text = "全选";
            mnuShowAliases.Text = "显示别名";
            mnuShowAllFields.Text = "显示全部字段";
            mnuShowSelected.Text = "仅显示选定形状";
            mnuStartEdit.Text = "开始编辑";
            mnuUpdateMeasurements.Text = "更新度量";
            mnuZoomToCurrentCell.Text = "缩放到正在编辑形状位置";
            toolEdit.Text = "编辑";
            toolFields.Text = "字段";
            toolJoin.Text = "加入数据源";
            toolLayout.Text = "布局";
            toolRemoveSelected.Text = "移除选定";
            toolSelection.Text = "选择";
            toolStopJoins.Text = "停止加入";
            toolTools.Text = "工具";
        }
        ...
        public void UpdatePanelCaption(int layerHandle)
        {
            ...
            if (fs != null)
            {
                ...
                if (fs.NumSelected > 0)
                {
                    msg += string.Format(" - 已选择: {0} / {1}", fs.NumSelected, fs.NumFeatures);
                }

                if (fs.CanEditTable())
                {
                    int modified = fs.Features.Count(ft => ft.RowModified);
                    msg += string.Format(" - 已更新: {0} / {1}", modified, fs.NumFeatures);
                }
            }
        ...
        private void UpdateMenus()
        {
            ...
            if (grid != null)
            {
                if (grid.RowManager.Filtered)
                {
                    ...
                    mnuShowSelected.Text = @"显示所有要素";
                }
                else
                {
                    mnuShowSelected.Text = @"显示已选择的要素";
                }
            }
