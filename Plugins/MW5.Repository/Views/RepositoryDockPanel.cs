...
namespace MW5.Plugins.Repository.Views
{
    ...
    {
        ToolStripItem mnuCreateOgrLayer;
        public RepositoryDockPanel(IRepository repository)
        {
            ...
            Init();
            mnuCreateOgrLayer = new ToolStripButton("新建图层", null, null, "mnuCreateOgrLayer");
            contextMenuStripEx1.Items.Add(mnuCreateOgrLayer);
        }
        ...
        private void SetDatabaseContextMenu()
        {
            contextMenuStripEx1.Items.AddRange(new ToolStripItem[]
                                                   { mnuRemoveConnection, new ToolStripSeparator(), mnuCreateOgrLayer, new ToolStripSeparator(), mnuRefresh });
        }

        private void SetDatabaseLayerContextMenu(IDatabaseLayerItem layer)
        {
            mnuAddToMap.Text = layer.AddedToMap ? "从地图文档中移除" : "添加到地图文档";
        ...
        {
            mnuAddToMap.Text = file.AddedToMap ? "从地图文档中移除" : "添加到地图文档";
        ...
        {
            Logger.Current.Debug("在更新描述");
            richTextBox1.SetText("加载中...");
            ...
            Task<string>.Factory.StartNew(item.GetDescription).ContinueWith(description =>
                {
                    ...
                    {
                        Logger.Current.Error("数据源说明加载失败: {0}", ex, item.DisplayName);
                        richTextBox1.SetText("说明加载失败。");
                    }
