...
namespace MW5.Plugins.TopologyChecker.Menu
{
    public class MenuUpdater: MenuServiceBase
    {
        ...
        public MenuUpdater(IAppContext context, TopologyCheckerPlugin plugin, IGeoprocessingService geoprocessingService):
        ...
        private void OnMapCursorChanged(IMuteMap sender, EventArgs e)
        {
            ...
            //FindToolbarItem(MenuKeys.CopyAttributes).Checked = map.MapCursor == MapCursor.CopyAttributes;
        ...
        private void OnViewUpdating(object sender, EventArgs e)
        {
            ...
            //editLayerItem.Description = editing ? "保存自上次保存起所做的所有编辑。保存后，无法撤销之前的编辑。" : "开始编辑会话以编辑要素或属性。";
            ...
            //    FindToolbarItem(MenuKeys.CopyAttributes).Enabled = selectedCount == 1;
            //    FindToolbarItem(MenuKeys.PasteAttributes).Enabled = _geoprocessingService.HasAttributesCopy();
