...
namespace MW5.Api.Legend
{
         ...
         where T : class, ILayer
    {
        ...
        public override T this[string name] => this.FirstOrDefault(l => l.Name == name);
        ...
        public int Add(ILayerSource layerSource, bool visible, bool legendVisible, int targetGroupHandle = -1, int positionInGroup = -1)
        {
            ...
            if (legendGroups != null)
            {
                ...
                if (targetGroupHandle == -1 && grp.Text == "拓扑检查")
                {
                    for (int i = 0; i < legendGroups.Count; i++)
                    {
                        if (legendGroups.GroupByLayerHandle(i) is LegendGroup group && group.Text != "拓扑检查")
                        {
                            grp = group;
                        }
                    }
