        ...
        public void AddLayer(ILegendLayer layer)
        {
            ...
            switch (layer.LayerType)
            {
                case Enums.LayerType.Image:
                    if (layer.Raster != null && layer.Raster.Bands.Count >= 3)
                    {
                        var msg = "º∆À„≤®∂Œ»°÷µ∑∂Œß";
                        GlobalListeners.ReportProgress(string.Empty, 0, msg);
                        for (int i = 1; i <= Math.Min(layer.ImageSource.NumBands, 4); i++)
                        {
                            GlobalListeners.ReportProgress(string.Empty, i * 50 / layer.ImageSource.NumBands, msg);
                            var band = layer.Raster.Bands[i];
                            //if (layer.Raster.NumBands != 3)
                            { 
                                //double min = band.Minimum, max = band.Maximum;
                                //band.ComputeMinMax(false, out min, out max);
                                var statics = band.GetStatistics(false, true);
                                layer.Raster.SetBandMinMax(i, statics.Min, statics.Max);
                            }
                            GlobalListeners.ReportProgress(string.Empty, i * 100 / layer.ImageSource.NumBands, msg);
                        }
                        GlobalListeners.ClearProgress();

                        layer.Raster.UseRgbBandMapping = true;
                    }
                    break;
            }
        }
