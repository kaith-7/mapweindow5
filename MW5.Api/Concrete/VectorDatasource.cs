...
namespace MW5.Api.Concrete
{
    public class VectorDatasource: IVectorDatasource
    {
        ...
        public bool CreateDatasource(string dsName, string ogrFormat)
        {
            return _datasource.CreateDataSource(dsName, ogrFormat);
        }
            ...
            get
            {
                ...
                foreach (var source in this.GetLayers())
                {
                    ...
                    if (layer != null)
                    {
                        ...
                        s += "图层名称: " + layer.Name + Environment.NewLine;
                        s += "数据几何类型: " + layer.GeometryType.EnumToString() +
                                             Environment.NewLine;
                        s += "数据包含要素数量: " + layer.get_FeatureCount() + Environment.NewLine;
