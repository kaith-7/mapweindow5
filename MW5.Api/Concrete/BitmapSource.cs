...
namespace MW5.Api.Concrete
{
    public class BitmapSource : IImageSource
    {
        ...
        public string SizeInfo 
        {
            get
            {
                return string.Format("[{0}×{1} 像素]", _image.Width, _image.Height);
            } 
        }
        ...
        public virtual string ToolTipText
        {
            get
            {
                string s = string.Format("分辨率: {0}×{1}", Width, Height) + Environment.NewLine;
                s += "中心点X坐标: " + XllCenter + Environment.NewLine;
                s += "中心点Y坐标: " + YllCenter + Environment.NewLine;
                s += "单位栅格长度: " + Dx + Environment.NewLine;
                s += "单位栅格宽度: " + Dy + Environment.NewLine;
                s += "投影: " + Projection.ExportToProj4();
                return s;
            }
        }
