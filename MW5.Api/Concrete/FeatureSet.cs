...
namespace MW5.Api.Concrete
{
    ...
    public class FeatureSet : IFeatureSet
    {
        ...
        /// </summary>
        internal FeatureSet(Shapefile sf, string unProjectedShapefile = "")
        {
            
            _shapefile = sf ?? throw new NullReferenceException("Shapefile reference is null.");

            var value = sf.CellValue[0, 0];
            var value1 = sf.CellValue[1, 0];
            var value2 = sf.CellValue[0, 1];
            //if (!string.IsNullOrEmpty(unProjectedShapefile))
            //{
            //    _shapefile.UnProjectedShapefile = unProjectedShapefile;
            //}
        }
        ...
        public string UnProjectedFilename
        {
            get => _shapefile.UnProjectedShapefile;
            set => _shapefile.UnProjectedShapefile = value;
        }
        ...
        public string ToolTipText
        {
            get
            {
                var s = "数据几何类型: " + GeometryType.EnumToString() + Environment.NewLine;
                s += "数据包含要素数量: " + Features.Count + Environment.NewLine;
                // s += "Projection: " + Projection.ExportToProj4();
                s += "投影: " + Projection.Name;
                return s;
            }
        }
        ...
        private IFeatureSet WrapShapefile(Shapefile sf, string unProjectedShapefile = null)
        {
            if (sf != null)
            {
                return new FeatureSet(sf, unProjectedShapefile);
            }
            return null;
        }
        ...
        public bool SaveAs(string shapefileName, bool switchToNew)
        {
            return _shapefile.SaveAs2(shapefileName, switchToNew);
        }
        ...
        public long UnProjectedID(int ShapeIndex)
        {
            return _shapefile.UnProjectedID[ShapeIndex];
        }

        public bool Deleted(int ShapeIndex)
        {
            return _shapefile.Deleted[ShapeIndex];
        }

        public Shape ReprojectSingle(ISpatialReference newProjection, int ShapeIndex)
        {
            var shp = _shapefile.ReprojectSingle(newProjection.GetInternal(), ShapeIndex);

            return shp;
        }

        public IFeatureSet Reproject(ISpatialReference newProjection, double[] transformParams, out int reprojectedCount)
        {
            int count = 0;
            
            var sf = _shapefile.Reproject(newProjection.GetInternal(), transformParams,  ref count);

            reprojectedCount = count;
            
            return WrapShapefile(sf, Filename);
        }
        ...
        public IFeatureSet AttributeCopy(IFeatureSet fsOverlay)
        {
            bool bSave = true;
            bool isEditingTable = this.EditingTable;
            try
            {
                if (!isEditingTable)
                {
                    this.StartEditingTable();
                }
                foreach (var item in this.Features)
                {
                    var list = new List<int>();
                    list.Add(item.Index);
                    this.UpdateSelection(list, SelectionOperation.New);
                    var fsFilter = this.ExportSelection();
                    int[] selectSet;
                    fsOverlay.SelectByShapefile(fsFilter, SpatialRelation.Contains, true, out selectSet);

                    if (selectSet.Length > 0)
                    {
                        var feat = fsOverlay.Features[selectSet[0]];
                        for (int i = 0; i < feat.NumFields; i++)
                        {
                            var field = feat.GetField(i);
                            var idxDst = this.Fields.IndexByName(field.Name);
                            if (field.Type == this.Fields[idxDst].Type)
                            {
                                var val = feat.GetValue(i);
                                item.SetValue(idxDst, val);
                            }
                        }
                    }
                }
