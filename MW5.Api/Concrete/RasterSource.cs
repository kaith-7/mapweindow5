...
using MW5.Shared;
using GdalUtils = MW5.Api.Static.GdalUtils;
...
namespace MW5.Api.Concrete
{
    public class RasterSource : BitmapSource, IRasterSource
    {
        ...
        public bool BuildOverviews(RasterOverviewSampling method, IEnumerable<int> scales, TiffCompression compression = TiffCompression.Auto)
        {
            ...
            _image.GlobalCallback = new GlobalNativeCallback();

            try
            {
                var ok = _image.BuildOverviews((tkGDALResamplingMethod)method, scales.Count(), scales.ToArray());
                //var ok = GdalUtils.Instance.GdalBuildOverviews(this.Filename, (tkGDALResamplingMethod)method, scales.ToArray());
                return ok;
            }
            catch (Exception ex)
            {
                GlobalListeners.ReportError("", ex.Message);
                return false;
            }
        }
