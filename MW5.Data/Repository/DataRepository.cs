...
namespace MW5.Data.Repository
{
    ...
    public class DataRepository : IRepository
    {
        ...
        public void RemoveFolderLink(string path, bool silent)
        {
            ...
            if (silent ||
                MessageService.Current.Ask("从存储库中删除此文件夹的链接?" +
                                           Environment.NewLine + "(该文件夹将在磁盘上保持不变。)"))
            {
        ...
        public void RemoveConnection(DatabaseConnection connection, bool silent)
        {
            ...
            {
                if (!MessageService.Current.Ask("删除数据库连接: " + connection.Name + "?"))
                {
                    return;
                }
            }
        ...
        private void Init()
        {
            ...
            TmsGroups = new RepositoryGroupList
                            {
                                new RepositoryGroup(TmsProvider.DefaultGroupId, "默认"),
                                new RepositoryGroup(TmsProvider.CustomGroupId, "自定义")
                            };
