    ...
    internal class WmsCapabilitiesPresenter : ComplexPresenter<IWmsCapabilitiesView, WmsCommand, WmsCapabilitiesModel>
    {
        ...
        private WmsServer SelectedServer
        {
            get
            {
                ...
                if (server == null)
                {
                    MessageService.Current.Info("未选择WMS服务。");
                    return null;
                }
        ...
        public override void RunCommand(WmsCommand command)
        {
            switch (command)
            {
                ...
                case WmsCommand.Delete:
                    if (View.Server != null)
                    {
                        string msg = "是否要删除所选的WMS服务: " + View.Server.Name + "?";
        ...
        /// <summary>
        /// Adds selected server to the map.  添加服务向导
        /// </summary>
        private void AddProvider()
        {
            ...
            if (layers.Count == 0)
            {
                MessageService.Current.Info("未选择WMS图层。");
            }
        ...
        /// <summary>
        /// Places WMS layer above data layers.  将WMS层放置在数据层上方。
        /// </summary>
        ...
        /// <summary>
        /// Sends GetCapabilities request to the specified URL. 发送url指令
        /// </summary>
        ...
        /// <summary>
        /// Displays capabilities (list of layers for the server).显示Capabilities（图层列表）
        /// </summary>
        ...
        /// <summary>
        /// Loads and displayes the cached capabilities for the server.   加载并显示服务器的缓存功能。
        /// </summary>
        ...
        /// <summary>
        /// Displayes and cached the results of GetCapabilities request.  显示并缓存GetCapabilities请求的结果。
        /// </summary>
        private void ProcessWmsCapabilitiesResults(Task<Stream> task)
        {
            var server = SelectedServer;

            string msg = "WMS服务访问失败: " + Environment.NewLine;
